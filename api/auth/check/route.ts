    ...
    try {
      ...
      return NextResponse.json({ 
        ...
        user: {
          ...
          points: user.profiles ? ((user.profiles as any).points || 0) : 0,
          totalXp: user.profiles ? ((user.profiles as any).total_xp || 0) : 0,
          level: user.profiles ? ((user.profiles as any).level || 1) : 1
        }
      })
    } catch (error) {
